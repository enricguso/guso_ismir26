...
def singalong_to_audience(musdbmoises_path):
    train_path = os.path.join(musdbmoises_path, 'train')
    train_path = Path(train_path)
    for song_dir in train_path.iterdir():
        if song_dir.is_dir():
            src = song_dir / "singalong.wav"
            if src.exists():
                src.rename(song_dir / "audience.wav")

def audience_to_singalong(musdbmoises_path):
    train_path = os.path.join(musdbmoises_path, 'train')
    train_path = Path(train_path)
    for song_dir in train_path.iterdir():
        if song_dir.is_dir():
            src = song_dir / "audience.wav"
            if src.exists():
                src.rename(song_dir / "singalong.wav")
...
def conv_torch(srcs, rirs):
    #Convolves stereo batches with stereo RIRs 
...
def augment_drr(rirs, drrs_factors):
    # Downscales the RIR with respect to the max abs
...
class ParirSetRIRS(Dataset):
    def __init__(self, main_path, subset, sample_rate=44100):
        self.maxlen = 60000 #maximum IR lenght in samples
        self.main_path = main_path
        self.subset = subset

        if subset == 'valid': subset = 'train'
        wav_files = []
        files = os.listdir(os.path.join(main_path, subset))
        for f in files:
            if '.wav' in f:
                wav_files.append(os.path.join(os.path.join(main_path, subset), f))
...
class CrowdiosetAmb_train(Dataset):
    def __init__(self, main_path, duration, sample_rate=44100):
        self.main_path = main_path
        self.duration = duration

        amb_files = os.listdir(os.path.join(main_path, 'train', 'ambiences'))
        amb_files.sort()
    ...
    def __getitem__(self, idx):
        ...
        amb, sr = torchaudio.load(os.path.join(self.main_path, 'train', 'ambiences', path_amb))
...
class CrowdiosetEve_train(Dataset):
    def __init__(self, main_path, duration,  sample_rate=44100):
        self.main_path = main_path
        self.duration = duration

        eve_files = os.listdir(os.path.join(main_path, 'train', 'events'))
        eve_files.sort()
    ...
    def __getitem__(self, idx):
        ...
        eve, sr2 = torchaudio.load(os.path.join(self.main_path, 'train', 'events', path_eve))
...
def main():
    print('Starting MUSDBMOISES+Crowdioset+PaRIRset training script...')
    ...
    rirset_train = ParirSetRIRS(config.data.rirs, subset='train')
    rirset_valid = ParirSetRIRS(config.data.rirs, subset='valid')
    ...
    crowdioset_train_amb = CrowdiosetAmb_train('/media/diskA/enric/crowdioset/', config.data.segment*config.data.samplerate, config.data.samplerate)
    crowdloader_amb_train = DataLoader(crowdioset_train_amb, batch_size=config.batch_size, shuffle=True, drop_last=True)

    crowdioset_train_eve = CrowdiosetEve_train('/media/diskA/enric/crowdioset/', config.data.segment*config.data.samplerate, config.data.samplerate)
    ...
    for epoch in range(oepoch, config.epochs):
        ...
        #rename the training singalongs to audience.wav to avoid changing SCNet dataloader
        singalong_to_audience(config.data.wav) 
        ...
        #rename the training audiences back to singalong.wav
        audience_to_singalong(config.data.wav) 
